...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // AJUSTE DEFINITIVO: Buscamos a primeira integração sem usar o campo 'nome'
    ...
    if (integracaoExistente) {
      ...
      // Se não existir nenhuma, criamos a primeira vinculada a um lojista
      const lojista = await prisma.lojista.findFirst();
      if (!lojista) throw new Error('Nenhum lojista cadastrado');
